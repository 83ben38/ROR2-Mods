...
public class DarknessExpansion : BaseUnityPlugin
{
    ...
    private void Awake()
    {
        ...
        beetleSpawnInterval     = Config.Bind("Kings Gland", "Spawn Interval",    30f,  "Seconds between Beetle Guard spawns");
        beetleBaseDamagePercent = Config.Bind("Kings Gland", "Guard Damage %",        300f, "Base % damage of each Beetle Guard");
        beetleBaseHealthPercent = Config.Bind("Kings Gland", "Guard Health %",        300f, "Base % health of each Beetle Guard");
        beetleDebuffStacks      = Config.Bind("Kings Gland", "Debuff Stacks per Hit",1,    "Number of debuff stacks Beetle applies on hit");
        beetleMaxGuards         = Config.Bind("Kings Gland", "Max Guards",            1,    "Maximum simultaneous Beetle Guards");
        beetleOnKillASPercent   = Config.Bind("Kings Gland", "Attack Speed Gain per Kill",         3f,   "Attack speed % gained on killing a dark enemy");
