...
public class RandomArtifact
{
    ...
    private void InventoryOnonServerItemGiven(Inventory arg1, ItemIndex arg2, int arg3)
    {
        ...
        if (optionNum == 0)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#FFFFFF>Nothing Happens!</color></style>"});
        }

        if (optionNum == 1)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#FF0000>Lose your item.</color></style>"});
            arg1.RemoveItem(arg2,arg3);
        }
        
        if (optionNum == 2)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#00FF00>Double your item.</color></style>"});
            arg1.itemStacks[arg1.itemAcquisitionOrder.IndexOf(arg2)] *= 2;
        }

        if (optionNum == 3)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#0000FF>Randomize your item.</color></style>"});
        ...
        if (optionNum == 4)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#00FF00>Upgrade your item.</color></style>"});
        ...
        if (optionNum == 5)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#FF0000>Downgrade your item.</color></style>"});
            ItemTier newTier = ItemTier.Boss;
        ...
        if (optionNum == 6)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#00FFFF>Get Bigger!</color></style>"});
            arg1.gameObject.transform.localScale *= 1.25f;
        }
        if (optionNum == 7)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#FF00FF>Get Smaller!</color></style>"});
            arg1.gameObject.transform.localScale *= .75f;
        }
        if (optionNum == 8)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#FF0000>Lose your Bead Stats.</color></style>"});
        ...
        if (optionNum == 9)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#00FF00>Get some Bonus Health!</color></style>"});
            arg1.beadAppliedHealth += 100f;
        }
        
        if (optionNum == 10)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#FFFF00>Get some Bonus Damage!</color></style>"});
            arg1.beadAppliedDamage += 5f;
        }

        if (optionNum == 11)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#00FFFF>Big Enemies Incoming!</color></style>"});
            bigEnemiesLeft += 10;
        }
        if (optionNum == 12)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#FF00FF>Small Enemies Incoming!</color></style>"});
            smallEnemiesLeft += 10;
        }

        if (optionNum == 13)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#00FF00>Heal to Full.</color></style>"});
        ...
        if (optionNum == 14)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#FF0000>Damage!!!!</color></style>"});
            CharacterBody cb = arg1.GetComponent<CharacterBody>();
        ...
        if (optionNum == 15)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#000000>Spinning the death roulette.</color></style>"});
            
            if (Random.RandomRangeInt(0, 100) == 0)
            {
                Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#000000>YOU HAVE BEEN DOOMED TO DIE!!!!</color></style>"});
                CharacterBody cb = arg1.GetComponent<CharacterBody>();
                for (int i = 0; i < 10; i++)
                {
        ...
        if (optionNum == 16)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#FF0000>An enemy equips this item.</color></style>"});
            itemToEquip = arg2;
        }
        if (optionNum == 17)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#00FF00>The next 5 elites will become normal enemies.</color></style>"});
            removeElites = 5;
        }
        if (optionNum == 18)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#0000FF>The next 5 normal enemies will become a random elite.</color></style>"});
            removeElites = 5;
        }
        if (optionNum == 19)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#FF00FF>Enable a random artifact.</color></style>"});
            RunArtifactManager.instance.SetArtifactEnabled(ArtifactCatalog.artifactDefs[(int)(ArtifactCatalog.artifactDefs.Length*Random.value)],true);
        }
        if (optionNum == 20)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#00FFFF>Gain a barrier!</color></style>"});
        ...
        if (optionNum == 21)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#FFAA00>Random teleport!</color></style>"});
        ...
        if (optionNum == 22)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#8888FF>Random Buff for 20 seconds!</color></style>"});
        ...
        if (optionNum == 23)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#FFFF00>A Meteor strikes the nearest foe!</color></style>"});
            var cb23 = arg1.GetComponent<CharacterBody>();
            CharacterBody nearest = Util.GetEnemyEasyTarget(cb23, new Ray(){direction = cb23.transform.forward}, 100,100F);
            if (nearest)
            {
                ...
                EffectManager.SpawnEffect(GlobalEventManager.CommonAssets.runicMeteorEffect, new EffectData { origin = nearest.transform.position }, true);
            }
        }
        
        if (optionNum == 24)
        {
            Chat.SendBroadcastChat(new Chat.SimpleChatMessage(){baseToken = "<style=cEvent><color=#FFFF00>Swap health with a random enemy!</color></style>"});
            var cb29 = arg1.GetComponent<CharacterBody>();
            var enemies = CharacterBody.instancesList.Where(b => b.teamComponent.teamIndex == TeamIndex.Monster).ToList();
            if (enemies.Count > 0)
            {
